...
const useTeacher = () => {
    ...
    try {
      if (editing) {
        const response = await fetch(
          `https://schooldashboard-production-04e3.up.railway.app/teachers/${editing.id}`,
          {
            method: "PUT",
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(`https://schooldashboard-production-04e3.up.railway.app/teachers/${id}`, {
        method: "DELETE",
      });
