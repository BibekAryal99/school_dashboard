...
const useTeacher = () => {
  ...
  useEffect(() => {
    const loadTeachers = async () => {
      try {
        const response = await fetch("https://blissful-cat-production.up.railway.app/teachers");
    ...
    try {
      if (editing) {
        const response = await fetch(
          `https://blissful-cat-production.up.railway.app/teachers/${editing.id}`,
          {
            method: "PUT",
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(`https://blissful-cat-production.up.railway.app/teachers/${id}`, {
        method: "DELETE",
      });
